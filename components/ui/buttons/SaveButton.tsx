...
export const SaveButton = ({ fullWidth = false }: SaveButtonProps) => {
        ...
        const result = await saveSite({
          sections: currentSections.map((section) => ({
            ...
            elements: section.elements.map((element) => ({
              ...element,
              id:
                element.id && !element.id.startsWith("temp_id")
                  ? element.id
                  : undefined,
            })),
          })),
  ...
  return (
    <button
      className={`flex items-center cursor-pointer gap-2 px-3 py-2 rounded-lg transition-all duration-300
        ...
        } border border-emerald-500/30 hover:border-emerald-500/50 ${fullWidth ? "w-full justify-center" : ""}`}
      onClick={handleSave}
      disabled={isSaving}
    >
