...
export const ItemRenderers: Record<string, React.FC<{ config:ConfigStyle, id:string } >> = {
  header: ({  config, id }) => {
  const openStylePanel = useCanvasStore((state) => state.openStylePanel);
    return (
    <header onClick={() => { openStylePanel(id) }} style={config} className="w-full h-full flex items-center ">
